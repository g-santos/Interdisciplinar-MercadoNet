...
public class Produto {
	...
	public Produto(int idproduto, String nome, String marcaprod, double preco, Categoria categoria) {
	...
	public static Produto pesquisarProduto(String produtonome) throws SQLException {
		Connection conn;
		try {
			conn = Conexao.getConnection();
			PreparedStatement stmt;
			...
			while (rs.next()) {
				produto = new Produto(rs.getInt("ID_PRODUTO"), rs.getString("NOME"), rs.getString("MARCAPROD"),
						rs.getDouble("PRECO"), Categoria.pesquisarCategoriaid(rs.getInt("ID_CATEGORIA")));
			}
			return produto;
		} catch (ClassNotFoundException Except) {
			// TODO Auto-generated catch block
	...
	public static Produto pesquisarProdutoid(int produtoid) throws SQLException {
		Connection conn;
		try {
			conn = Conexao.getConnection();
			PreparedStatement stmt;
	...
	public static void alterarProduto() throws Exception {
		...
		Produto p = pesquisarProdutoid(idproduto);
